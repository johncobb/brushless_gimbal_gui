...
class CpSerialResult:
    ResultCode = 0
    Data = ""
    
    
class CpSerial(threading.Thread):
    ...
    def __init__(self, modemResponseCallbackFunc=None, *args):
        ...
        self.modemResult = CpSerialResult()
        self.modemToken = ""
        ...
        while not self.closing:
            ...
            while(self.ser.inWaiting() > 0):
                ...
                if(tmp_char == '\r'):
                    ...
                    #if(result.ResultCode > CpSerialResultCode.RESULT_UNKNOWN):
                        #print 'known result code', result
                        
                    if(self.modemResponseCallbackFunc != None):
                        self.modemResponseCallbackFunc(tmp_buffer)
                        self.modemBusy = False

                    #print 'serial response ', tmp_buffer
                    #self.enqueue_result(tmp_buffer)
                    tmp_buffer= ""
                else:
                    tmp_buffer += tmp_char
            #self.__lock.release()
    ...
    def enqueue_result(self, result):   
        try:
            self.queue.put(result, block=True, timeout=1)
        except:
            self.__lock.acquire()
    ...
    def is_error(self, token):        
        if(token.find(CpSerialResponses.TOKEN_ERROR) > -1):
            return True
        else:
            return False
    ...
    def modem_parse_result(self, result):
        ...
        modem_result = CpSerialResult()
        
        if(result.find(CpSerialResponses.TOKEN_OK) > -1):
            modem_result.Data = result
            modem_result.ResultCode = CpSerialResultCode.RESULT_OK
        elif(result.find(CpSerialResponses.TOKEN_ERROR) > -1):
            modem_result.Data = result
            modem_result.ResultCode = CpSerialResultCode.RESULT_ERROR
        else:
            modem_result.Data = result
            modem_result.ResultCode = CpSerialResultCode.RESULT_UNKNOWN
    ...
    def modem_send_at(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_AT)
    ...
    def modem_set_echo_off(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETECHOOFF)
        self.modemResponseCallbackFunc = callback
        pass
        
    def modem_set_interface(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SELINT)
        self.modemResponseCallbackFunc = callback
        pass
        
    def modem_set_msg_format(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETMSGFMT)
        self.modemResponseCallbackFunc = callback
        pass
    
    def modem_set_band(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETBAND)
        self.modemResponseCallbackFunc = callback
        pass 
        
    def modem_set_context(self, callback):
        cmd = CpSerialDefs.CMD_SETCONTEXT % (CpDefs.Apn)
        self.enqueue_command(cmd)
        self.modemResponseCallbackFunc = callback
        pass   
        
    def modem_set_user_id(self, callback):
        cmd = CpSerialDefs.CMD_SETUSERID % (CpDefs.ApnUserid)
        self.enqueue_command(cmd)
        self.modemResponseCallbackFunc = callback
        pass
        
    def modem_set_password(self, callback):
        cmd = CpSerialDefs.CMD_SETPASSWORD % (CpDefs.ApnPassword)
        self.enqueue_command(cmd)
        self.modemResponseCallbackFunc = callback
        pass
        
    def modem_set_skipescape(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETSKIPESC)
        self.modemResponseCallbackFunc = callback
        pass   
        
    def modem_set_socket_config(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETSKTCFG)
        self.modemResponseCallbackFunc = callback
        pass
        
    def modem_set_autoactctx(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETAUTOCTX)
        self.modemResponseCallbackFunc = callback
        pass
    
    def modem_set_activatecontext(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETACTCTX)
        self.modemResponseCallbackFunc = callback
        pass 
    
    def modem_set_deactivatecontext(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETDACTCTX)
        self.modemResponseCallbackFunc = callback
        pass 
    
    def modem_qry_context(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_QRYCTX)
        self.modemResponseCallbackFunc = callback
        pass  
    
    def modem_qry_signal(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_QRYSIG)
        self.modemResponseCallbackFunc = callback
        pass   
    
    def modem_qry_network(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_QRYNET)
        self.modemResponseCallbackFunc = callback
        pass   
    
    def modem_set_factoryreset(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETFACTRST)
        self.modemResponseCallbackFunc = callback
        pass
    
    def modem_set_networkmonitoring(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SETMONI)
        self.modemResponseCallbackFunc = callback
        pass    
    
    def modem_socketdial(self, callback):
        cmd = CpSerialDefs.CMD_SKTDIAL % (CpDefs.Port, CpDefs.Server)
        self.enqueue_command(cmd)
        self.modemResponseCallbackFunc = callback
        pass  
   
    def modem_socketsuspend(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SKTESC)
        self.modemResponseCallbackFunc = callback
        pass
     
    def modem_socketresume(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SKTRESUME)
        self.modemResponseCallbackFunc = callback
        pass
    
    def modem_socketclose(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_SKTCLOSE)
        self.modemResponseCallbackFunc = callback
        pass  
    
    def modem_sendhttp(self, callback):
        self.enqueue_command(CpSerialDefs.CMD_HTTP)
        self.modemResponseCallbackFunc = callback
        pass  
    
    def modem_sendpost(self, callback, data):
        cmd = CpSerialDefs.CMD_HTTPPOST % (CpDefs.ServerFolder, CpDefs.Server, len(data), data)
        self.enqueue_command(cmd)
        self.modemResponseCallbackFunc = callback
        pass 
